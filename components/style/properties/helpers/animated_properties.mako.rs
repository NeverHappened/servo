...
/// on an existing transform list.
/// http://dev.w3.org/csswg/css-transforms/#none-transform-animation
fn build_identity_filter(_list: &AnimatedFilter) -> Result<AnimatedFilter, ()> {
    unimplemented!()
}

fn animate_scale_factors(
    a: CSSFloat,
    b: CSSFloat,
    procedure: Procedure,
) -> Result<CSSFloat, ()> {
    let a = a - 1.;
    let b = b - 1.;
    Ok(a.animate(&b, procedure)? + 1.)
}

/// Add two transform lists.
/// http://dev.w3.org/csswg/css-transforms/#interpolation-of-transforms
fn animate_transform_lists(
    from_list: &[TransformOperation],
    to_list: &[TransformOperation],
    procedure: Procedure,
) -> TransformList {
    if !can_interpolate_list(from_list, to_list) {
        let (_, other_weight) = procedure.weights();
        let from_list = TransformList(Some(from_list.to_vec()));
        let to_list = TransformList(Some(to_list.to_vec()));
        return TransformList(Some(vec![TransformOperation::InterpolateMatrix {
                from_list,
                to_list,
                progress: Percentage(other_weight as f32),
        }]));
    }
    ...
    for (from, to) in from_list.iter().zip(to_list) {
        match (from, to) {
            (
                &TransformOperation::Matrix(ref from),
                &TransformOperation::Matrix(ref to),
            ) => {
                let sum = from.animate(to, procedure).unwrap();
            ...
            (
                ...
                // FIXME: We don't animate `-moz-transform` matrices yet.
            ...
            (
                ...
                let ix = fx.animate(&tx, procedure).unwrap();
                let iy = fy.animate(&ty, procedure).unwrap();
            ...
            (
                ...
                let ix = fx.animate(&tx, procedure).unwrap();
                let iy = fy.animate(&ty, procedure).unwrap();
                let iz = fz.animate(&tz, procedure).unwrap();
            ...
            (
                ...
                let ix = animate_scale_factors(fx, tx, procedure).unwrap();
                let iy = animate_scale_factors(fy, ty, procedure).unwrap();
                let iz = animate_scale_factors(fz, tz, procedure).unwrap();
            ...
            (
                ...
                if fx == tx && fy == ty && fz == tz {
                    let ia = fa.animate(&ta, procedure).unwrap();
                    ...
                    let sum = matrix_f.animate(&matrix_t, procedure).unwrap();
                    result.push(TransformOperation::Matrix(sum));
                }
            }
            (
                ...
                let sum = fd_matrix.animate(&td_matrix, procedure).unwrap();
                result.push(TransformOperation::Matrix(sum));
            }
            _ => {
...
impl Animate for InnerMatrix2D {
    fn animate(&self, other: &Self, procedure: Procedure) -> Result<Self, ()> {
        Ok(InnerMatrix2D {
            m11: animate_scale_factors(self.m11, other.m11, procedure)?,
            m12: self.m12.animate(&other.m12, procedure)?,
            m21: self.m21.animate(&other.m21, procedure)?,
            m22: animate_scale_factors(self.m22, other.m22, procedure)?,
        })
    }
}
...
impl Animate for Scale2D {
    fn animate(&self, other: &Self, procedure: Procedure) -> Result<Self, ()> {
        Ok(Scale2D(
            animate_scale_factors(self.0, other.0, procedure)?,
            animate_scale_factors(self.1, other.1, procedure)?,
        ))
    }
}
...
impl Animate for Scale3D {
    fn animate(&self, other: &Self, procedure: Procedure) -> Result<Self, ()> {
        Ok(Scale3D(
            animate_scale_factors(self.0, other.0, procedure)?,
            animate_scale_factors(self.1, other.1, procedure)?,
            animate_scale_factors(self.2, other.2, procedure)?,
        ))
    }
}
...
impl Animate for Perspective {
    fn animate(&self, other: &Self, procedure: Procedure) -> Result<Self, ()> {
        Ok(Perspective(
            ...
            self.1.animate(&other.1, procedure)?,
            self.2.animate(&other.2, procedure)?,
            animate_scale_factors(self.3, other.3, procedure)?,
        ))
    }
}
...
impl Animate for MatrixDecomposed3D {
    ...
    fn animate(&self, other: &Self, procedure: Procedure) -> Result<Self, ()> {
        ...
        if other_weight == 1.0 {
            ...
            let w = (other_weight as f32 * theta).sin() * 1.0 / (1.0 - product * product).sqrt();

            let mut a = *self;
            let mut b = *other;
            % for i in range(4):
                a.quaternion.${i} *= (other_weight as f32 * theta).cos() - product * w;
                b.quaternion.${i} *= w;
...
impl Animate for TransformList {
    ...
    fn animate(&self, other: &Self, procedure: Procedure) -> Result<Self, ()> {
        if procedure == Procedure::Add {
            // FIXME(nox): Where is that defined in any spec?
            let this = self.0.as_ref().map_or(&[] as &[_], |this| &**this);
            let other = self.0.as_ref().map_or(&[] as &[_], |this| &**this);
            return Ok(TransformList(Some(this.iter().chain(other).cloned().collect())));
        }

        // http://dev.w3.org/csswg/css-transforms/#interpolation-of-transforms
        match (self.0.as_ref(), other.0.as_ref()) {
            (Some(this), Some(other)) => {
                if let Procedure::Accumulate { count } = procedure {
                    if !can_interpolate_list(this, other) {
                        // FIXME(nox): Where is that defined in any spec?
                        return Ok(TransformList(Some(vec![TransformOperation::AccumulateMatrix {
                            from_list: TransformList(Some(this.clone())),
                            to_list: TransformList(Some(other.clone())),
                            count: cmp::min(count, i32::max_value() as u64) as i32,
                        }])));
                    }
                }
                Ok(animate_transform_lists(this, other, procedure))
            },
            (Some(this), None) => {
                // http://dev.w3.org/csswg/css-transforms/#none-transform-animation
                let other = build_identity_transform_list(this);
                Ok(animate_transform_lists(this, &other, procedure))
            },
            (None, Some(other)) => {
                if procedure.is_accumulate() {
                    // Accumulating `none` and then adding `other` equals `other`.
                    return Ok(TransformList(Some(other.clone())));
                }
                // http://dev.w3.org/csswg/css-transforms/#none-transform-animation
                let this = build_identity_transform_list(other);
                Ok(animate_transform_lists(&this, other, procedure))
            },
            (None, None) => {
...
fn animate_filter_function_impl(
    this: &AnimatedFilter,
    other: &AnimatedFilter,
    procedure: Procedure,
) -> Result<AnimatedFilter, ()> {
    match (this, other) {
        % for func in ['Blur', 'HueRotate', 'Grayscale', 'Invert', 'Sepia']:
        ...
        (&Filter::${func}(ref this), &Filter::${func}(ref other)) => {
            Ok(Filter::${func}(NonNegative(
                animate_scale_factors(this.0, other.0, procedure)?,
            )))
        },
        % endfor
        % if product == "gecko":
        (&Filter::DropShadow(ref this), &Filter::DropShadow(ref other)) => {
            Ok(Filter::DropShadow(from_value.animate(other, procedure)?))
        },
...
/// https://drafts.fxtf.org/filters/#animation-of-filters
fn animate_filter_function(
    this: Option<<&AnimatedFilter>,
    other: Option<<&AnimatedFilter>,
    procedure: Procedure,
) -> Result<AnimatedFilter, ()> {
    match (this, other) {
        (Some(f), Some(t)) => {
            animate_filter_function_impl(f, t, procedure)
        },
        (Some(this), None) => {
            let other = build_identity_filter(this)?;
            animate_filter_function_impl(this, &other, procedure)
        },
        (None, Some(other)) => {
            let this = build_identity_filter(other)?;
            animate_filter_function_impl(&this, other, procedure)
        },
        _ => { Err(()) }
    }
...
impl Animate for AnimatedFilterList {
    ...
    fn animate(&self, other: &Self, procedure: Procedure) -> Result<Self, ()> {
        ...
        // FIXME(nox): This can be rewritten more beautifully with itertools.
        let mut from = from_iter.next();
        let mut to = to_iter.next();
        while from.is_some() || to.is_some() {
            filters.push(animate_filter_function(from, to, procedure)?);
            if from.is_some() {
                from = from_iter.next();
            }
            if to.is_some() {
        ...
        self.0.iter().zip_longest(other.0.iter()).map(|it| {
            match it {
                ...
                EitherOrBoth::Left(list) | EitherOrBoth::Right(list) => {
                    let none = build_identity_filter(list)?;
