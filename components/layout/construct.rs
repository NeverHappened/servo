    ...
    FlowConstructor<'a, ConcreteThreadSafeLayoutNode>
{
    ...
    fn flush_inline_fragments_to_flow(
        ...
        let mut fragments = fragment_accumulator
            .to_intermediate_inline_fragments::<ConcreteThreadSafeLayoutNode>(
                self.style_context(),
                self.layout_context.show_debug_fragment_borders,
            );
        ...
        inline_flow_ref.finish(bubble_inline_sizes_separately);
                ...
                if flow.is_table() && kid_flow.is_table_caption() {
                        ...
                        .contains(FlowFlags::IS_ABSOLUTELY_POSITIONED)
                    {
                        ...
                        self.flush_inline_fragments_to_flow(
                            ...
                            legalizer,
                            node,
                            self.layout_context.bubble_inline_sizes_separately,
                        );
                    }
                    legalizer.add_child::<ConcreteThreadSafeLayoutNode::ConcreteElement>(
                        self.style_context(),
    ...
    fn build_flow_for_block_starting_with_fragments(
        ...
        flow.finish(self.layout_context.bubble_inline_sizes_separately);
    ...
    fn build_flow_for_table(
        ...
        wrapper_flow.finish(self.layout_context.bubble_inline_sizes_separately);
...
impl FlowRef {
    ...
    fn finish(&mut self, bubble_inline_sizes_separately: bool) {
...
impl Legalizer {
        ...
        E: TElement,
    {
        while !self.stack.is_empty() {
            ...
            self.flush_top_of_stack(parent, bubble_inline_sizes_separately)
        }

        while !self.try_to_add_child::<E>(
            context,
            parent,
            &mut child,
            bubble_inline_sizes_separately,
    ...
    fn finish(mut self, parent: &mut FlowRef, bubble_inline_sizes_separately: bool) {
        while !self.stack.is_empty() {
            self.flush_top_of_stack(parent, bubble_inline_sizes_separately)
        }
    }
        ...
        E: TElement,
    {
        ...
        match (parent_class, child_class) {
            ...
            (FlowClass::Flex, FlowClass::Inline) => {
                ...
                block_wrapper.finish(bubble_inline_sizes_separately);
    ...
    fn flush_top_of_stack(&mut self, parent: &mut FlowRef, bubble_inline_sizes_separately: bool) {
        let mut child = self.stack.pop().expect("flush_top_of_stack(): stack empty");
        child.finish(bubble_inline_sizes_separately);
